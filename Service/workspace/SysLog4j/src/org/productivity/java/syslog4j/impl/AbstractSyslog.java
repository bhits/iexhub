...
public abstract class AbstractSyslog implements SyslogIF {
	...
	protected void write(SyslogMessageProcessorIF messageProcessor, int level, String message) throws SyslogRuntimeException {
		...
		header = (m.length + h.length) + " " + header;
		byte[] h2 = SyslogUtility.getBytes(this.syslogConfig, header);

		int availableLen = this.syslogConfig.getMaxMessageLength() - /*h.length*/ h2.length;
		...
		if (mLength <= availableLen) {
			byte[] data = messageProcessor.createPacketData(h2,m,0,mLength);
