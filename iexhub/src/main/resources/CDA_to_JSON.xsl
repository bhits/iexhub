...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:strip-space elements="text content paragraph item list caption td "/>
        ...
        <xsl:for-each select="@*">
            ...
            <xsl:choose>
                ...
                <xsl:when test="$attr-name='ID'">
                    <xsl:copy-of select="."/>
                    <!-- check value  (<xsl:value-of select="."/>)
                    -->
                </xsl:when>
                <xsl:when
...
<xsl:template match="n1:text">
    ...
    <xsl:template match="n1:text">
        <xsl:choose>
            <xsl:when test="text()and (count(child::n1:table)= 0) and (count(child::n1:content) = 0) and (count(child::n1:list) = 0) and (count(child::n1:paragraph) = 0)">  
                <xsl:variable name="t1"><xsl:value-of select="translate(text(),'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:variable name="t2"><xsl:value-of select=
                    "substring($t1,
                    string-length(
                    substring-before($t1,
                    substring(translate($t1, ' ', ''),
                    1,
                    1)
                    )
                    ) +1
                    ...
                    "/></xsl:variable> 
                "text":"<xsl:value-of select="$t2"/>"
               
            </xsl:when>
            <xsl:when test="(count(child::n1:table)> 0) or (count(child::n1:content) > 0) 
                or (count(child::n1:list) > 0) or (count(child::n1:paragraph) > 0)">
                <xsl:variable name="t1"><xsl:value-of select="translate(.,'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:variable name="t2"><xsl:value-of select=
                    "substring($t1,
                    string-length(
                    substring-before($t1,
                ...
                <xsl:if test="count(child::n1:paragraph) &gt; 0">
                    
                   "paragraph":[
                    <xsl:apply-templates select="n1:paragraph"/>
                    ]
                </xsl:if>
                <xsl:if test="count(child::n1:renderMultiMedia) &gt; 0">
                    <xsl:if test="count(child::n1:paragraph) &gt; 0" >
                        ,
                    </xsl:if>
                 
                    <xsl:apply-templates select="n1:renderMultiMedia"></xsl:apply-templates>
                    
                    </xsl:if>
                <xsl:if test="count(child::n1:content) &gt; 0 ">                    
                    <xsl:if test="(count(child::n1:paragraph) &gt; 0) or 
                        (count(child::n1:renderMultiMedia) &gt; 0)" >
                        ,
                    </xsl:if>
                    "content":[
                ...
                <xsl:if test="count(child::n1:list) &gt; 0">
                    <xsl:if test="(count(child::n1:paragraph) &gt; 0) 
                        or (count(child::n1:content) &gt; 0)" >
                        ,
                    </xsl:if>
                    "list":[
                       
                            <xsl:apply-templates select="n1:list"/>
                        
                    ] 
                </xsl:if>
                
                
                <xsl:if test="count(child::n1:table) &gt; 0">
                    <!--   
                        <xsl:if test="(count(child::n1:content) &gt; 0) or (count(child::n1:list) &gt; 0) 
                            or (count(child::n1:paragraph) &gt; 0) or (string-length($t2) &gt; 0)">
                        ,
                    </xsl:if>  
                    -->       
                    <xsl:if test="(count(child::n1:paragraph) &gt; 0) 
                        or (count(child::n1:content) &gt; 0)
                        or (count(child::n1:list) &gt; 0)" >
                        ,
                    </xsl:if>
                    "table":
                    [
                    <xsl:apply-templates select="n1:table"/>  
                    ]
                </xsl:if>
    ...
    <xsl:template
        match="n1:table | n1:thead | n1:tfoot | n1:tbody | n1:colgroup | n1:col | n1:tr | n1:th | n1:td | n1:list| n1:item |n1:content">
       ...
       <!-- add only the id -->
        <xsl:choose>
            <xsl:when test="not(@ID)">
               <!-- skip if there is no text or id -->
                <xsl:variable name="t1"><xsl:value-of select="translate(.,'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:variable name="t2"><xsl:value-of select=
                    "substring($t1,
                    string-length(
                    ...
                    "/></xsl:variable> 
                <xsl:if test="string-length($t2)>0">                
                    {
                    <xsl:if test="@ID">"id":"<xsl:value-of select="@ID"/>",  </xsl:if>
                    "text":"<xsl:value-of select="$t2"/>"                    
                    }
                    <xsl:if test="count(following-sibling::n1:content) &gt; 0" >
                        ,
                    </xsl:if>
                </xsl:if>
                </xsl:when>
            <xsl:when test="text() and (count(child::n1:content)= 0)">
                   
                    {
                    <xsl:if test="@ID">"id":"<xsl:value-of select="@ID"/>",  </xsl:if>              
                    "text":"<xsl:value-of select="translate(text(),'&#xA;&#xD;&#x9;', '')"/>"                
                    }                    
                <xsl:if test="count(following-sibling::n1:content) &gt; 0" >
                        ,
                    </xsl:if>          
                 </xsl:when>            
            <xsl:when test="(count(child::n1:content)&gt; 0) or (not(@ID))">
                <xsl:if test="@ID">"id":"<xsl:value-of select="@ID"/>",  </xsl:if>
                "content":[
                <xsl:apply-templates select="n1:content"/>
                ]
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="t2"><xsl:value-of select=
                    ...
                    {
                    <xsl:if test="@ID">"id":"<xsl:value-of select="@ID"/>"  </xsl:if>
                <xsl:if test="@ID and (string-length($t2)>0)">,</xsl:if>
                    <xsl:if test="string-length($t2)>0">"text":"<xsl:value-of select="$t2"/>"  </xsl:if> 
                     }                
                <xsl:if test="count(following-sibling::n1:content) &gt; 0" >                  ,
                </xsl:if>
            </xsl:otherwise>                
    ...
    <xsl:template match="n1:paragraph">
        ...
        <xsl:choose>
            <xsl:when test="."> 
                <xsl:variable name="inText"><xsl:value-of select="translate(text(),'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:if test="string-length($inText) &gt; 0">
                    "text":"<xsl:value-of select="$inText"/>"
                </xsl:if>
                <xsl:if test="count(child::n1:content)>0">
                    ,<xsl:apply-templates select="n1:content"/>
                </xsl:if>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="count(child::n1:paragraph)>0">
                    <xsl:apply-templates select="n1:pragraph"/>
                </xsl:if>
            </xsl:otherwise>
            </xsl:choose>  -->
        <xsl:choose>
            <xsl:when test="count(child::n1:content) = 0">
                <xsl:variable name="t1">
                <xsl:for-each select="child::text()">
                    <xsl:choose>
                        <xsl:when test="self::n1:br" >
                ...
                <xsl:if test="string-length($t1)">
                "text":  "<xsl:value-of select="$t1"/>"   
                </xsl:if>
                <xsl:if test="count(child::n1:renderMultiMedia)&gt; 0">
                    ...
                    <xsl:apply-templates select="n1:renderMultiMedia"/>
        ...
        <xsl:if test="count(following-sibling::n1:paragraph) &gt; 0" >
            ,
        </xsl:if>
    </xsl:template>
    <!-- List -->
    ...
    <xsl:template match="n1:item">
        ...
        <xsl:choose>
            <xsl:when test="count(child::*)>0"> 
                <!-- replace(., '^\s+|\s+$', '') 
                    <xsl:variable name="t2"><xsl:value-of select="translate(.,'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                -->
                <xsl:variable name="t2"><xsl:value-of select="translate(text(),'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:variable name="t1"><xsl:value-of select=
                    "substring($t2,
                    string-length(
                    substring-before($t2,
                    substring(translate($t2, ' ', ''),
                    1,
                    1)
                    )
                    ) +1
                    ...
                    "/></xsl:variable> 
                <xsl:variable name="t0"><xsl:value-of select="translate($t1, '  ',' ')"/></xsl:variable>
                  <xsl:if test="string-length($t0)>0">                     
                    "text":"<xsl:value-of select="$t0"/>"
                </xsl:if>
                <xsl:if test="count(child::n1:content)>0">
                    <xsl:if test="string-length($t0)>0"> ,</xsl:if>
                    <xsl:if test="count(child::n1:content)">
                        "content":[
                    <xsl:apply-templates select="n1:content"/>
                        ]
                    </xsl:if>
            ...
            <xsl:otherwise>
                <xsl:variable name="v1"><xsl:value-of select="translate(.,'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
                <xsl:variable name="v2"><xsl:value-of select=
                    "substring($v1,
                    string-length(
                    substring-before($v1,
                    substring(translate($v1, ' ', ''),
                    1,
                    1)
                    )
                    ) +1
    ...
    <xsl:template match="n1:table">
        {      
        <xsl:if test="count(n1:thead) &gt; 0">
            "thead":[
            <xsl:apply-templates select="n1:thead"/>
            ]
            
            <xsl:if test="count(child::n1:tbody) &gt; 0" >
                ,
            </xsl:if>
        </xsl:if>     
        <xsl:if test="count(n1:tbody) &gt; 0">
        "tbody":[
            <xsl:apply-templates select="n1:tbody"/>
        ]
            <xsl:if test="count(child::n1:footnote) &gt; 0" >
                ,
            </xsl:if>
        </xsl:if>
        
       
        <xsl:apply-templates select="n1:footnote"/>
        }
        <xsl:if test="count(following-sibling::n1:table) &gt; 0" >
            ,
        </xsl:if>        
    </xsl:template>
    <xsl:template match="n1:thead">
        {        
                "tr":[                
                <xsl:apply-templates select="n1:tr"/>
            ]
        }
        <xsl:if test="count(following-sibling::n1:thead) &gt; 0" >
            ,
        </xsl:if>
    </xsl:template>
    <xsl:template match="n1:tfootnote">
        "tfootnote":{
            <xsl:apply-templates/>
        }
    </xsl:template>
    ...
    <xsl:template match="n1:tbody">        
        ...
        <xsl:if test="count(following-sibling::n1:tbody) &gt; 0" >
            ,
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="n1:tr">
        {            
            "th":[
              ...
              <xsl:apply-templates select="n1:th" />
    ...
    <xsl:template match="n1:td">
        ...
        <!-- tdlast  
        position: <xsl:value-of select="position()"/> ouf of <xsl:value-of select="last()"/>
        -->
        <xsl:if test="count(following-sibling::n1:td) &gt; 0" >
            ,
        </xsl:if>
    ...
    <xsl:template match="n1:th">
        {
        <!--   
            text()trim: '<xsl:value-of select="translate(text(),'&#xA;&#xD;&#x9;', '')"/>'
            .trim:
        -->
        <xsl:variable name="t1"><xsl:value-of select="translate(.,'&#xA;&#xD;&#x9;', '')"/></xsl:variable>
        <xsl:variable name="t2"><xsl:value-of select=
        ...
        <!-- tdlast  
            position: <xsl:value-of select="position()"/> ouf of <xsl:value-of select="last()"/>
        -->
        <xsl:if test="count(following-sibling::n1:th) &gt; 0" >
            ,
        </xsl:if>
        
    </xsl:template>
    <xsl:template match="n1:caption">
        "caption":{"text":"<xsl:apply-templates/>"}        
        <xsl:if test="count(following-sibling::*) &gt; 0" >
            ,
        </xsl:if>
 ...
 @mediaType='image/jpeg']">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            ...
                            "referencedObject":"<xsl:value-of select="$image-uri"/>"
                            <xsl:if test="count(child::n1:caption)">
                                ,<xsl:apply-templates select="n1:caption"/>
                            </xsl:if>                                    
                        </xsl:otherwise>
                    </xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:if test="//n1:observationMedia[@ID=$imageRef]/n1:value/@mediaType">
                    ,
                    "mediaType":"<xsl:value-of select="//n1:observationMedia[@ID=$imageRef]/n1:value/@mediaType"/>"
                </xsl:if>
                
                <xsl:if test="count(child::n1:caption) &gt; 0">
                    ,
                    <xsl:apply-templates select="n1:caption"></xsl:apply-templates>
                </xsl:if>
